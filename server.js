...
app.put('/shopping-list/:id', jsonParser, (req, res) => {
    ...
    const updatedItem = ShoppingList.update({id: req.params.id, name: req.body.name, budget: req.body.budget});
...
app.put('/recipes/:id', jsonParser, (req,res) => {
  ...
  if (name in req.body || id in req.body && req.params.id === req.body.id) {
    console.log(`Updating Recipes, with id ${req.params.id}`);
    const update = {
      id: req.params.id,
      name: req.body.name,
      ingredients: req.body.ingredients
    };
    const updateRecipe = Recipes.update(update);
    res.status(200).json(updateRecipe);

  } else if (req.params.id !== req.body.id) {
    const message = `The parameter ${req.params.id} did not match ${req.body.id}`;
  } else if (!name in req.body || !id in req.body) {
    const message = `No ${name} or ${id} in the body`;
    console.log(message);
    return res.status(400).send(message);
  }
});
